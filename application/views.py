...
def myblog(request):
    ...
    user = request.user
    return render(request, "userblog.html", context={"blog_qset": blog_list, "user": user})
...
def blog_content(request, title):
    ...
    if request.method=="POST":
        ...
        if len(content) > 0:
            comment = CommentModel(user = user, content = content, blog=blog)
            comment.save()
        return HttpResponseRedirect(request.path)
    
    comments = CommentModel.objects.filter(blog=blog)
    return render(request, "blogdetail.html", context={"blog": blog, "comments": comments})
